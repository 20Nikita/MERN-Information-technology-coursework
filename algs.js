const WS_Clock = (РазмерБуффера, КоличествоЭлементов, КоличествоСтраниц,
    ...
    масив.Буффер[0].БитИзменения[0] = Содержимое[0].ВремяОбращения
    масив.Буффер[0].ВремяПоследнегоИзменения[0] = 0

    let столбец = 1
    let Clok = 1
    let k = 0
    for (let i = 1; i < КоличествоЭлементов; i++) {
        ...
        let круг = 1
        if(масив.Pf[столбец] == 0){
            for (let j = 0; j < РазмерБуффера; j++) {
                (Clok + j < РазмерБуффера - 1)? k = Clok + j : k = (Clok + j) - РазмерБуффера
                console.log(масив.Буффер)
                скопировать(столбец,i)
                console.log(масив.Буффер)
            ...
            if(круг){
                ...
                //
                скопировать(столбец,i)
                if(масив.Буффер[Clok].БитИзменения[столбец] == 0) {
